...
class SimpleMemory(BaseMemory):
    ...
    def store_selected(self, record: Dict[str, List[Any]], indices: List[int]):
        """
        Store a new record for a subset of environments.
        """
        if self.keys is None:
            self.keys = list(record.keys())
        assert self.keys == list(record.keys())
        ...
        for local_idx, env_idx in enumerate(indices):
            self._data[env_idx].append({k: record[k][local_idx] for k in self.keys})
