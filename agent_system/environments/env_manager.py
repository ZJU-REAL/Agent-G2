...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def replay_prefix_plans(self, prefix_plans, collect_sft: bool = False, collect_detail: bool = False) -> Tuple[Dict[str, Any], np.ndarray, np.ndarray, np.ndarray]:
        ...
        for step_idx in range(max_prefix_steps):
            ...
            if collect_detail:
                available_actions_snapshot = [
                    ...
                    for commands in self.envs.get_admissible_commands
                ]
            ...
            text_obs, _, rewards, dones, infos = self.envs.step_selected(selected_indices, selected_actions)
        ...
        image_obs = self.envs.getobs() if getattr(self.envs, "multi_modal", False) else None
        next_observations = {'text': full_text_obs, 'image': image_obs, 'anchor': self.pre_text_obs.copy()}
        return next_observations, prefix_rewards, prefix_lengths, prefix_dones
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        ...
        prefix_info = None
        if hasattr(self, "prefix_terminal_infos") and self.prefix_terminal_infos is not None:
...
class WebshopEnvironmentManager(EnvironmentManagerBase):
    ...
    def replay_prefix_plans(self, prefix_plans, collect_sft: bool = False, collect_detail: bool = False):
        ...
        for step_idx in range(max_prefix_steps):
            ...
            previous_text_obs = [self.pre_text_obs[env_idx] for env_idx in selected_indices]
            text_obs, rewards, dones, infos = self.envs.step_selected(selected_indices, selected_actions)
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        ...
        if hasattr(self, "prefix_terminal_infos") and self.prefix_terminal_infos is not None:
            prefix_info = self.prefix_terminal_infos[batch_idx]
        if prefix_info is not None:
            won_value = float(prefix_info.get('won', False))
            score_value = float(prefix_info.get('task_score', 0.0))
            success['success_rate'].append(won_value)
            success['webshop_task_score (not success_rate)'].append(score_value)
            return
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        env_kwargs = {
                    ...
                    'file_path': file_path,
                    'attr_path': attr_path,
                    'train_goal_source': config.env.webshop.get('train_goal_source', 'default'),
                    'train_sample_mode': config.env.webshop.get('train_sample_mode', 'random'),
                    'train_expert_json_path': config.env.webshop.get('train_expert_json_path', None),
                    'val_exclude_train_goals': config.env.webshop.get('val_exclude_train_goals', False),
                    }
