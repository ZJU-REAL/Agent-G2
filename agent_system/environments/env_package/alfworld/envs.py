...
class AlfworldWorker:
    ...
    def __init__(self, config, seed, base_env):
        ...
        self.env = self._init_env(gamefile=None)

    def _init_env(self, gamefile=None):
        if gamefile is None:
            env_builder = self.base_env
        else:
            env_builder = copy.copy(self.base_env)
            env_builder.game_files = [gamefile]
            env_builder.num_games = 1
        env = env_builder.init_env(batch_size=1)
        env.seed(self.seed)
        return env

    def _switch_gamefile(self, gamefile=None):
        if gamefile == self.current_gamefile:
            return
        if hasattr(self.env, "close"):
            self.env.close()
        self.env = self._init_env(gamefile=gamefile)
    ...
    def reset(self, gamefile=None):
        ...
        self._switch_gamefile(gamefile=gamefile)
...
class AlfworldEnvs(gym.Env):
    ...
    def step_selected(self, indices, actions):
        assert len(indices) == len(actions), \
            "The num of actions must match the num of selected worker indices"

        futures = []
        for local_idx, worker_idx in enumerate(indices):
            future = self.workers[worker_idx].step.remote(actions[local_idx])
            futures.append(future)
        ...
        results = ray.get(futures)
        for local_idx, (obs, scores, dones, info) in enumerate(results):
            for k in info.keys():
                info[k] = info[k][0]

            worker_idx = indices[local_idx]
            text_obs_list.append(obs[0])
            dones_list.append(dones[0])
            info_list.append(info)
    ...
    def reset(self, kwargs=None):
        ...
        image_obs_list = []
        info_list = []
        gamefiles = [None for _ in range(self.num_processes)]
        if kwargs is not None:
            if isinstance(kwargs, dict):
                shared_gamefile = kwargs.get("gamefile") or kwargs.get("extra.gamefile")
                gamefiles = [shared_gamefile for _ in range(self.num_processes)]
            else:
                for i, item in enumerate(kwargs):
                    if i >= self.num_processes:
                        break
                    if isinstance(item, dict):
                        gamefiles[i] = item.get("gamefile") or item.get("extra.gamefile")

        # Send reset commands to all workers
        futures = []
        for i, worker in enumerate(self.workers):
            future = worker.reset.remote(gamefiles[i])
            futures.append(future)
    ...
    def reset_selected(self, indices, kwargs=None):
        text_obs_list = []
        image_obs_list = []
        info_list = []
        gamefiles = [None for _ in indices]
        if kwargs is not None:
            if isinstance(kwargs, dict):
                shared_gamefile = kwargs.get("gamefile") or kwargs.get("extra.gamefile")
                gamefiles = [shared_gamefile for _ in indices]
            else:
                for i, item in enumerate(kwargs):
                    if i >= len(indices):
                        break
        ...
        futures = []
        for local_idx, worker_idx in enumerate(indices):
            future = self.workers[worker_idx].reset.remote(gamefiles[local_idx])
            futures.append(future)

        results = ray.get(futures)
        for local_idx, (obs, info) in enumerate(results):
            for k in info.keys():
                info[k] = info[k][0]
            worker_idx = indices[local_idx]
            text_obs_list.append(obs[0])
    ...
    def getobs_selected(self, indices):
        futures = []
        for worker_idx in indices:
            future = self.workers[worker_idx].getobs.remote()
            futures.append(future)
