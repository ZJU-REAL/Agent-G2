...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_output_gen_batch = self.traj_collector.multi_turn_loop(
                                                    ...
                                                    rollout_detail_dump_dir=self.config.trainer.get("val_rollout_detail_data_dir", None),
                                                    )
    ...
    def _save_checkpoint(self):
        ...
        prefix_runtime = getattr(self.traj_collector, "prefix_runtime", None)
        if prefix_runtime is None:
            prefix_runtime = getattr(self.traj_collector, "gmsv_runtime", None)
        if prefix_runtime is not None:
            prefix_runtime.save_to_checkpoint(local_global_step_folder)
    ...
    def _active_prefix_cfg(self):
        for config_key in ("gmsv", "binary_search", "enumerate_hint", "cosine_hint", "liner_hint", "fix_step_hint", "fix_acc_hint", "trapo"):
            prefix_cfg = self.config.get(config_key, {})
            if bool(prefix_cfg.get("enable", False)):
                return prefix_cfg
        return {}

    def _prepare_prefix_sft_actor_batch(self, batch: DataProto, metrics: dict) -> DataProto:
        prefix_sft_cfg = self._active_prefix_cfg().get("prefix_sft", {})
        if not bool(prefix_sft_cfg.get("enable", False)):
            return batch

        prefix_sft_batch = getattr(self.traj_collector, "latest_prefix_sft_batch", None)
        if prefix_sft_batch is None or len(prefix_sft_batch) == 0:
            return batch
        if "multi_modal_inputs" in batch.non_tensor_batch:
            metrics["prefix_sft/skipped_multimodal"] = 1.0
            return batch

        actor_keys = ["responses", "input_ids", "attention_mask", "position_ids", "old_log_probs", "advantages"]
        if self.config.actor_rollout_ref.rollout.multi_turn.enable and "loss_mask" in batch.batch.keys():
            actor_keys.append("loss_mask")
        if self.config.actor_rollout_ref.actor.use_kl_loss and "ref_log_prob" in batch.batch.keys():
            actor_keys.append("ref_log_prob")

        rl_actor_batch = batch.select(batch_keys=actor_keys, non_tensor_batch_keys=[])
        response_shape = rl_actor_batch.batch["responses"].shape
        rl_actor_batch.batch["prefix_sft_mask"] = torch.zeros(response_shape, dtype=torch.bool)
        rl_actor_batch.batch["prefix_sft_step_weight"] = torch.zeros(response_shape[0], dtype=torch.float32)
        rl_actor_batch.batch["prefix_sft_padding_mask"] = torch.zeros(response_shape[0], dtype=torch.bool)

        if self.config.actor_rollout_ref.actor.use_kl_loss and "ref_log_prob" in actor_keys and "ref_log_prob" not in prefix_sft_batch.batch.keys():
            prefix_sft_batch.batch["ref_log_prob"] = torch.zeros_like(prefix_sft_batch.batch["responses"], dtype=torch.float32)
        if "prefix_sft_padding_mask" not in prefix_sft_batch.batch.keys():
            prefix_sft_batch.batch["prefix_sft_padding_mask"] = torch.zeros(len(prefix_sft_batch), dtype=torch.bool)

        sft_keys = actor_keys + ["prefix_sft_mask", "prefix_sft_step_weight", "prefix_sft_padding_mask"]
        if any(key not in prefix_sft_batch.batch.keys() for key in sft_keys):
            missing = [key for key in sft_keys if key not in prefix_sft_batch.batch.keys()]
            raise KeyError(f"prefix SFT batch is missing required actor keys: {missing}")

        sft_actor_batch = prefix_sft_batch.select(batch_keys=sft_keys, non_tensor_batch_keys=[])
        actor_batch = DataProto.concat([rl_actor_batch, sft_actor_batch])
        pad_size = (-len(actor_batch)) % self.actor_rollout_wg.world_size
        if pad_size:
            pad_batch = actor_batch[:1].repeat(pad_size)
            ...
            if "loss_mask" in pad_batch.batch.keys():
                pad_batch.batch["loss_mask"] = torch.zeros_like(pad_batch.batch["loss_mask"])
            if "ref_log_prob" in pad_batch.batch.keys():
                pad_batch.batch["ref_log_prob"] = torch.zeros_like(pad_batch.batch["ref_log_prob"])
            actor_batch = DataProto.concat([actor_batch, pad_batch])
        ...
        actor_batch.meta_info["prefix_sft_enabled"] = True
        actor_batch.meta_info["prefix_sft_loss_coef"] = float(prefix_sft_cfg.get("loss_coef", 1.0))
        actor_batch.meta_info["prefix_sft_loss_type"] = str(prefix_sft_cfg.get("loss_type", "ce"))
        actor_batch.meta_info["prefix_sft_clip_low"] = float(prefix_sft_cfg.get("clip_low", 0.1))

        metrics["prefix_sft/num_samples"] = float(len(prefix_sft_batch))
        metrics["prefix_sft/num_tokens"] = float(prefix_sft_batch.batch["prefix_sft_mask"].sum().item())
        return actor_batch
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        gen_batch_output = self.traj_collector.multi_turn_loop(
                                                                ...
                                                                is_train=True,
                                                                global_step=self.global_steps,
                                                                rollout_detail_dump_dir=self.config.trainer.get("train_rollout_detail_data_dir", None),
                                                                )
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if prefix_runtime is None:
                            prefix_runtime = getattr(self.traj_collector, "gmsv_runtime", None)
                        if prefix_runtime is not None:
                            prefix_metrics = prefix_runtime.update_after_train_batch(batch.non_tensor_batch)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer("update_actor", timing_raw):
                            actor_batch = self._prepare_prefix_sft_actor_batch(batch, metrics)
