...
def _distributed_backend() -> str:
    ...
    visible_npu = os.environ.get("ASCEND_RT_VISIBLE_DEVICES")
    raise RuntimeError(
        "No CUDA or NPU backend is available in this worker. "
        f"CUDA_VISIBLE_DEVICES={visible_cuda}, ASCEND_RT_VISIBLE_DEVICES={visible_npu}. "
        "If this is a CUDA run, stop Ray and restart with CUDA_VISIBLE_DEVICES set."
    )


def _nccl_backend() -> str:
    if torch.cuda.is_available():
        return "nccl"
    if is_npu_available:
        return "hccl"
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(backend=_distributed_backend(), rank=rank, world_size=world_size)
...
class CriticWorker(Worker):
    def __init__(self, config):
        ...
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(backend=_nccl_backend())
        self.config = config
