...
class FixAccHintAlfworldPrefixRuntime:
    def __init__(self, tokenizer, config):
        self.tokenizer = tokenizer
        self.config = config
        self.fix_acc_hint_cfg = config.fix_acc_hint
        self.rng = np.random.default_rng(int(self.fix_acc_hint_cfg.get("seed", 1)))
        self.store = self._load_expert_store()
        self.loaded_global_step = 0
        self.train_batches_completed = 0
        self.drop_count = max(int(self.fix_acc_hint_cfg.get("initial_drop_count", 0)), 0)
    ...
    def _expert_json_path(self) -> str:
        configured_path = self.fix_acc_hint_cfg.get("expert_json_path")
    ...
    def _load_expert_store(self) -> ExpertTrajectoryStore:
        ...
        num_groups = int(self.fix_acc_hint_cfg.get("num_difficulty_groups", 5))
    ...
    def _state_file_path(self, checkpoint_folder: str) -> str:
        return os.path.join(checkpoint_folder, "fix_acc_hint_runtime_state.json")
    ...
    def _load_persisted_state(self, checkpoint_folder: str) -> bool:
        ...
        self.drop_count = int(state.get("drop_count", self.drop_count))
        return True
    ...
    def _update_drop_count(self, batch_accuracy: float, matched_rollout_count: int) -> bool:
        if matched_rollout_count <= 0:
            return False
        if batch_accuracy < float(self.fix_acc_hint_cfg.get("acc_threshold", 0.7)):
            return False
    ...
    def update_after_train_batch(self, non_tensor_batch: dict[str, Any]) -> dict[str, float]:
        ...
        expert_matched = np.asarray(non_tensor_batch.get("fix_acc_hint_expert_matched", np.ones(len(rollout_ids), dtype=bool)), dtype=bool)
        ...
        prefix_steps = np.asarray(non_tensor_batch.get("fix_acc_hint_prefix_step_count", []), dtype=np.float32)
        ...
        drop_count_before = int(self.drop_count)
        updated = self._update_drop_count(
            batch_accuracy=batch_accuracy,
            matched_rollout_count=matched_rollout_count,
        )
        metrics = {
            "fix_acc_hint/train_step": float(current_step),
            "fix_acc_hint/drop_count": float(drop_count_before),
            "fix_acc_hint/drop_count_after_update": float(self.drop_count),
            "fix_acc_hint/acc_threshold": float(self.fix_acc_hint_cfg.get("acc_threshold", 0.7)),
            "fix_acc_hint/threshold_updated": float(updated),
            "fix_acc_hint/matched_prompt_count": float(matched_rollout_count),
            "fix_acc_hint/batch_prompt_accuracy": batch_accuracy,
        }
        if prefix_steps.size > 0:
            metrics["fix_acc_hint/avg_prefix_step_count"] = float(prefix_steps.mean())
            metrics["fix_acc_hint/max_prefix_step_count"] = float(prefix_steps.max())
    ...
    def build_batch_metadata(self, plans: list[PrefixPlan]) -> dict[str, np.ndarray]:
        return {
            "fix_acc_hint_trial_id": np.array([plan.trial_id for plan in plans], dtype=object),
            "fix_acc_hint_expert_matched": np.array([plan.matched for plan in plans], dtype=bool),
            "fix_acc_hint_task_type": np.array([plan.task_type for plan in plans], dtype=object),
            "fix_acc_hint_difficulty_group": np.array([plan.difficulty_group for plan in plans], dtype=np.int64),
            "fix_acc_hint_initial_prefix_step_count": np.array([plan.mu for plan in plans], dtype=np.float32),
            "fix_acc_hint_drop_count": np.array([self.drop_count for _ in plans], dtype=np.int64),
            "fix_acc_hint_hint_ratio": np.array([plan.clipped_ratio for plan in plans], dtype=np.float32),
            "fix_acc_hint_prefix_step_count": np.array([plan.prefix_step_count for plan in plans], dtype=np.int64),
            "fix_acc_hint_prefix_token_count": np.array([plan.prefix_token_count for plan in plans], dtype=np.int64),
            "fix_acc_hint_total_step_count": np.array([plan.total_step_count for plan in plans], dtype=np.int64),
            "fix_acc_hint_total_token_count": np.array([plan.total_token_count for plan in plans], dtype=np.int64),
            "fix_acc_hint_fixed_no_prefix_train": np.array([plan.fixed_no_prefix_train for plan in plans], dtype=bool),
        }


def build_fix_acc_hint_runtime(tokenizer, config) -> Optional[FixAccHintAlfworldPrefixRuntime]:
