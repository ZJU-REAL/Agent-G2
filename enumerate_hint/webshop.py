...
class EnumerateHintWebshopPrefixRuntime(EnumerateHintAlfworldPrefixRuntime):
    """WebShop prefix runtime for online enumerate-hint search."""
    ...
    def _load_expert_store(self) -> WebshopExpertTrajectoryStore:
        ...
        if skipped:
            print(f"[EnumerateHint WebShop] skipped {skipped} expert trajectories without target key")
        print(f"[EnumerateHint WebShop] loaded {len(store_items)} unique expert target keys from {json_path}")
    ...
    def step_count_for_ratio(self, trial_id: str, gamefile: str, ratio: float) -> int:
        target_key = normalize_target_key(gamefile) or normalize_target_key(trial_id)
            ...
            for info in group_infos[1:]:
                ...
                if group_target_key != target_key:
                    raise ValueError(
                        ...
                        "enumerate_hint.share_prefix_within_group=true; got "
