...
def run_sft_only(config) -> None:
    ...
    runner = SFTOnlyTaskRunner.remote()
    ray.get(runner.run.remote(config))


@ray.remote(num_cpus=1)
class SFTOnlyTaskRunner:
    def run(self, config):
        ...
        trainer = build_sft_only_trainer(config, local_path=local_path)
        trainer.init_workers()
        trainer.fit()


def build_sft_only_trainer(config, local_path=None, build_envs=True):
