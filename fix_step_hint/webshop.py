...
from fix_step_hint.alfworld import FixStepHintAlfworldPrefixRuntime
...
class FixStepHintWebshopPrefixRuntime(FixStepHintAlfworldPrefixRuntime):
    ...
    def _load_expert_store(self) -> WebshopExpertTrajectoryStore:
        ...
        if skipped:
            print(f"[FixStepHint WebShop] skipped {skipped} expert trajectories without target key")
        print(f"[FixStepHint WebShop] loaded {len(store_items)} unique expert target keys from {json_path}")
            ...
            for info in group_infos[1:]:
                ...
                if group_target_key != target_key:
                    raise ValueError(
                        ...
                        "fix_step_hint.share_prefix_within_group=true; got "
